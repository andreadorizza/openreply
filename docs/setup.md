...
`comments` carries comment-to-DM, which is what most people come here for. The messaging fields carry inbound DMs and Story replies (the "also reply when someone DMs these words" toggle), button taps (the step after the opening DM), and read receipts (the fallback that sends that step anyway when someone reads the opening DM without tapping). Subscribe to `comments` alone and those toggles look enabled but never fire, because the events they need are never delivered.

Meta also keeps a per-account field list, which OpenReply registers when you connect an Instagram account. If you suspect webhooks are not reaching a connected account, use the "Re-sync webhooks" button in Settings — it re-registers the account and reports which fields Meta actually has active, without a disconnect/reconnect.
